...
/**
 * <p>
 * A TileFactory is responsible for finding and/or creating tiles for a given
 * TileService and area. Here we make no distinction whereas a requested tile is
 * created or retrieved form an internal cache.
 * </p>
 * <p>
 * When creating/finding a tile, the factory must know the zoom level for which
 * the tile is required. Also, a geographic position or, alternatively a
 * reference tile, must be passed.
 * </p>
 * 
...
public abstract class TileFactory {
     ...
     * @return a tile
     */
    public abstract Tile findTileAtCoordinate(double lon, double lat,
    ...
    /**
     * Finds the tile for a service at the given position and zoom level, which
     * is immediately to the right of the passed tile.
     * 
    ...
    /**
     * Finds the tile for a service at the given position and zoom level, which
     * is immediately below the the passed tile.
     * 
     * @param tile the reference tile
     * @param service
     * @return
    ...
    /**
     * Some clients, e.g. uDig, may produce numbers like -210° for the
     * longitude, but we need a number in the range -180 to 180, so instead of
     * -210 we want 150.
     *
